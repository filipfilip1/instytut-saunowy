...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Find training
    const training = await Training.findBySlug(params.slug);
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: 'payment',
      success_url: `${process.env.NEXT_PUBLIC_APP_URL}/szkolenia/${params.slug}/success?session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: `${process.env.NEXT_PUBLIC_APP_URL}/szkolenia/${params.slug}`,
    ...
    // TODO: Send confirmation email after successful payment
    // Integrate with emailService from EMAIL_INVOICE_SETUP.md
