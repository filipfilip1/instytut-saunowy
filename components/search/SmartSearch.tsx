...
export default function SmartSearch({
  ...
  const debouncedQuery = useDebounce(query, 300); // Add 300ms debounce
    ...
    <div ref={wrapperRef} className={`relative ${className}`}>
      ...
      <div className="relative">
        <input
          ...
          className="w-full px-4 py-2 pl-10 pr-4 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-warmwood-500 focus:border-transparent"
        />
      ...
      {showDropdown && (
        <div className="absolute top-full left-0 right-0 mt-2 bg-white shadow-lg rounded-lg border border-gray-200 z-50 max-h-96 overflow-y-auto">
          {/* Search Results */}
          {debouncedQuery.length >= MIN_SEARCH_LENGTH && results.length > 0 && (
            <div>
          ...
          {debouncedQuery.length >= MIN_SEARCH_LENGTH && results.length === 0 && (
            <div className="p-4 text-center text-gray-500">
              <p className="mb-1">Nie znaleziono produktów dla &ldquo;{debouncedQuery}&rdquo;</p>
          ...
          {query.length > 0 && query.length < MIN_SEARCH_LENGTH && (
            <div className="p-4 text-center text-gray-500">
              <p className="text-sm">Wpisz co najmniej {MIN_SEARCH_LENGTH} znaki aby wyszukać</p>
            </div>
          )}
            ...
            <div>
              <div className="flex items-center justify-between px-4 py-2 border-b">
                ...
                <button
                  ...
                  className="text-xs text-warmwood-600 hover:text-warmwood-700"
                >
                  Wyczyść
                </button>
              </div>
