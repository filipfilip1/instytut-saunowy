...
import { Product, ProductVariant, VariantOption } from '../../types/Product';
...
const Cart: React.FC = () => {
  ...
  const getVariantDisplay = (product: Product, selectedVariants: Record<string, string>) => {
    const displays: string[] = [];

    product.variants.forEach((variant: ProductVariant) => {
      const selectedOptionId = selectedVariants[variant.id];
      if (selectedOptionId) {
        const option = variant.options.find((opt: VariantOption) => opt.id === selectedOptionId);
        if (option) {
